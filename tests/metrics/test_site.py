...
class TestSiteCreateSnapshot:
    ...
    def test_snapshot_data_includes_site_config(self):
        config = {"formFactor": "desktop", "throttling": "devtools"}
        site = SiteFactory(config=config)
        snapshot = site.create_snapshot()
        assert snapshot.data["config"] == config

    def test_config_file_is_created_on_disk(self):
        site = SiteFactory(config={"formFactor": "mobile"})
        snapshot = site.create_snapshot()
        assert os.path.exists(snapshot.data["config_file"])

    def test_config_file_contains_the_site_config(self):
        config = {"formFactor": "desktop"}
        site = SiteFactory(config=config)
        snapshot = site.create_snapshot()
        with open(snapshot.data["config_file"]) as fp:
