"""Test with Celery eager mode and actual tasks - inside tests dir."""
import pytest
...
def test_kaleido_with_celery_tasks(httpserver, settings, tmpdir):
    ...
    print('report path:', snapshot.report.path if snapshot.report else 'no report')
    assert snapshot.report.path
